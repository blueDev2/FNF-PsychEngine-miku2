...
class PlayState extends MusicBeatState
{
	...
	var mikuLights:FlxTypedGroup<BGSprite>;
	var curLight:Int;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'mikuStage':
				...
				var sky:BGSprite = new BGSprite('miku/sky', -100, 0, 0.1, 0.1);
				add(sky);

				var bgCity:BGSprite = new BGSprite('miku/Bgcity',-40,0,0.3,0.3);
				add(bgCity);

				var city:BGSprite = new BGSprite('miku/city', -40, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);

				mikuLights = new FlxTypedGroup<BGSprite>();
				add(mikuLights);

				for (i in 0...5)
				{
					var light:BGSprite = new BGSprite('miku/win' + i, city.x, city.y, 0.3, 0.3);
					light.visible = false;
					light.setGraphicSize(Std.int(light.width * 0.85));
					light.updateHitbox();
					mikuLights.add(light);
				}

				var streetBehind:BGSprite = new BGSprite('miku/behindTrain', -70, 50);
				add(streetBehind);
				
				var daStage:BGSprite = new BGSprite('miku/stage', -70, 50);
				add(daStage);

				var floor:BGSprite = new BGSprite('miku/Miku_floor',-70,50);
				add(floor);

				var miku_specatator:BGSprite = new BGSprite('miku/miku_specatator',-380,40,1,1,["bop"],true);
				miku_specatator.setGraphicSize(Std.int(miku_specatator.width*0.7));
				miku_specatator.animation.getByName("bop").frameRate = 8;
				add(miku_specatator);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'mikuStage':
				mikuLights.members[curLight].alpha -= (Conductor.crochet / 1000) * FlxG.elapsed * 1.5;
		}
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'mikuStage':
				if (curBeat % 4 == 0)
				{
					mikuLights.forEach(function(light:BGSprite)
					{
						light.visible = false;
					});

					curLight = FlxG.random.int(0, mikuLights.length - 1, [curLight]);

					mikuLights.members[curLight].visible = true;
					mikuLights.members[curLight].alpha = 1;
				}
		}
